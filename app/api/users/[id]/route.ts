...
import { updateUser, deleteUser } from '@/repositories/UserRepository';
...
export async function PUT(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    if (!name && !email && !role) {
      return new NextResponse('Bad Request: At least one field (name, email, role) must be provided for update', { status: 400 });
    }
